...
description: A ski marketing page I built to play with scroll animations and chase that wow effect. Every motion runs on native CSS scroll-driven timelines — no JavaScript — and I've rebuilt each one here as a live preview you can scrub yourself.
pubDate: 2026-06-15
...
> Browser support note: scroll-driven timelines are in Chromium and Firefox today; Safari is still catching up ([check current support on caniuse](https://caniuse.com/mdn-css_properties_animation-timeline)). The whole site is built so that without timeline support — and for anyone with `prefers-reduced-motion: reduce` — you get the full, readable, static page. Motion is an enhancement, never a dependency.
