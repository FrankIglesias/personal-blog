...
```css
body { timeline-scope: --trav; }     /* hoist the name up to a common ancestor */

.traverse { view-timeline: --trav block; }   /* defined way down here */

.topbar {
  position: fixed;
  mix-blend-mode: difference;
  animation: topbar-lime linear both;
  animation-timeline: --trav;        /* a fixed header attaches to a section's timeline */
...
.statement-tip::after {
  ...
  mask-image: url("/images/kore93-tip.webp");   /* clip the glow to the ski */
...
.blu .d {
  stroke-dasharray: 1;          /* with pathLength="1", the dash = the whole path */
  stroke-dashoffset: 1;         /* ...pushed fully "off" → invisible */
  animation: draw linear both;
  animation-timeline: --blu;
}
@keyframes draw { to { stroke-dashoffset: 0; } }   /* draw to 0 → fully inked */
...
```css
.showcase {
  height: 640svh;                /* tall section = the scroll "runway" */
  view-timeline: --show block;
}
.show-stage {
  position: sticky;
  top: 0;
  height: 100svh;                /* pinned frame the action plays inside */
}
.ski-l {
  animation: ski-l-seq linear both;
  animation-timeline: --show;
  animation-range: contain;      /* run only while the stage is fully pinned */
}
@keyframes ski-l-seq {
  ...
  74.4%,100%  { transform: ...scaleX(0); }   /* edge-on, hands off to the base ski */
}
```
...
@keyframes bs2-seq {
  ...
  38.4%,61.6% { opacity: 1; transform: none; }       /* hold in the middle third */
...
The whole thing is one stylesheet and zero JavaScript. If you want to poke at the real keyframes, it's all in [the repo](https://github.com/FrankIglesias/head-kore-93) — or [scroll the live page](https://head-kore-93.vercel.app/) and watch all twelve fire in sequence.
